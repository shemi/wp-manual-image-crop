...
class MicSettingsPage
{
    ...
    public function sizes_settings_callback()
    {
		...
		foreach ($imageSizes as $s) {
			...
			$quality = isset($sizesSettings[$s]['quality']) ? $sizesSettings[$s]['quality'] : '80';
			$visibility = isset($sizesSettings[$s]['visibility']) ? $sizesSettings[$s]['visibility'] : 'visible';
			$customLabel = isset($sizesSettings[$s]['label']) ? $sizesSettings[$s]['label'] : '';
			
			echo '<tr>
			     <td>' . $label. '</td>
			     <td><select name="mic_options[sizes_settings][' . $s . '][visibility]">
     					<option value="visible">' . __('Yes', 'microp') . '</option>
     					<option value="hidden" ' . ( $visibility == 'hidden' ? 'selected' : '' ) . '>' . __('No', 'microp') . '</option>
    				</select></td>
			     <td><select name="mic_options[sizes_settings][' . $s . '][quality]">
     					<option value="100">' . __('100 (best quality, biggest file)', 'microp') . '</option>
     					<option value="80" ' . ( $quality == '80' ? 'selected' : '' ) . '>' . __('80 (very high quality)', 'microp') . '</option>
     					<option value="70" ' . ( $quality == '70' ? 'selected' : '' ) . '>' . __('70 (high quality)', 'microp') . '</option>
     					<option value="60" ' . ( $quality == '60' ? 'selected' : '' ) . '>' . __('60 (good)', 'microp') . '</option>
     					<option value="50" ' . ( $quality == '50' ? 'selected' : '' ) . '>' . __('50 (average)', 'microp') . '</option>
     					<option value="30" ' . ( $quality == '30' ? 'selected' : '' ) . '>' . __('30 (low)', 'microp') . '</option>
     					<option value="10" ' . ( $quality == '10' ? 'selected' : '' ) . '>' . __('10 (very low, smallest file)', 'microp') . '</option>
    				</select></td>
			     <td><input name="mic_options[sizes_settings][' . $s . '][label]" type="text" placeholder="' . $label . '" value="' . str_replace('"', '&quot;', $customLabel) .  '"/></td>
			</tr>';
		}
		echo '</tbody></table>';
